...
```{r}
table(prob = pred2_class[[1]], class = pred[[1]])
```

Results aren't identical because we used differently tuning parameters.

```{r}
bind_rows(
  mutate(tune$best_tune, type = "class"),
  mutate(tune2$best_tune, type = "prob")
)
```

What does it look like through an endpoint?

```{r}
sagemaker_deploy_endpoint(tune2)

sagemaker_has_endpoint(tune2)
```

```{r}
xgb <- sagemaker_load_model(tune2)
```

```{r}
predict(xgb, abalone_class[, -1])
```



```{r}
endpoint_pred <- predict(tune2,  abalone_class[, -1])
```
...
## TODO: need to write unti test to ocmpare this to local predict method


json_pred <- endpoint_pred %>%
  as.character() %>%
  stringr::str_c("[", ., "]") %>%
  jsonlite::parse_json()
...
json_pred %>%
  ...
  tibble(.pred = .) %>%
  unnest_wider(.pred, names_repair = "minimal")
```


```{r}
"1,2,3,4" %>%
  format_endpoint_predictions()
```
